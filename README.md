...
4 ana işi var:
1. Port Tarama Hedef sistemde hangi portların açık olduğunu tarar. 3.6 saniyede 1024 portu kontrol edip açık olanları listeler.

2. Banner Grabbing Açık portlardaki servislerin kendini nasıl tanıttığına bakar. Örneğin micro_httpd bilgisini buradan alır. Bu bilgiyle "bu servisin bilinen bir açığı var mı" diye bakılır.

3. Keşif (Recon) Hedef bir domain ise DNS kayıtlarını çeker, WHOIS ile kimin adına kayıtlı olduğuna bakar, subdomain brute-force ile gizli alt domainleri bulmaya çalışır.

4. Zafiyet Kontrolü Bulduğu servis versiyonlarını CVE veritabanıyla karşılaştırır. Örneğin "bu FTP sunucusu vsFTPd 2.3.4, bunda backdoor var" gibi uyarılar verir.
